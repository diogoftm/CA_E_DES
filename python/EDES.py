...
class SBOXESGenerator:
    ...
    def generate(key):
        ...
        for i in range(0, 4095):
            ...
            j = ((int(derivedKey[2*(i+1)-1]) << 8) +
                 int(derivedKey[2*i])) % 4096
...
class EDES:
    ...
    def __fN(val: bytes, sbox: bytes):
        assert len(
            val) == 8, "Invalid input size for Feistel Network. Condition: len(input)==8 bytes"
        l: bytes = val[:4]
        r: bytes = val[4:]
    ...
    def __processBlock(self, block: bytes, reverseFlag: bool):
        assert len(
            block) == 8, "Invalid block size. Condition: len(block)!=8 bytes"
        result = block
        for i in range(0, 16):
            if not reverseFlag:
    ...
    def encrypt(self, plaintext: bytes):
        assert len(
            plaintext) % 8 == 0, "Invalid plaintext size. Condition: len(data)%8 == 0"
