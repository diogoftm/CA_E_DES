...
E-DES uses a 256 bit key, maintains the block size, removes both permutation boxes and the creation of the S-Boxes depends only on the key. In this report we go into 
...
Idealy the number of confusion bits should be block\_size/2. For a 64 bit block cipher (such as E-DES), 32 is the number to ideally expect. A script has developed in order
...
Idealy the number of diffusion bits should be block\_size/2. For a 64 bit block cipher (such as E-DES), 32 is the number to ideally expect. A script has developed in order
...
Our C++ E-DES is around 10\% slower than Open SSL's DES, when tested in a AMD Ryzen 7 7735HS with 16Gb of RAM. E-DES encryption values almost don't fluctuate but
DES times go up and down even sometimes surpassing E-DES's on average. This result is encouraging because we're comparing our proof-of-concept implementation of E-DES with Open 
...
on the key. We proved its feasibility by developing two implementation of the algorithm and by performing various. E-DES have shown to be 
a clearly viable cipher.
